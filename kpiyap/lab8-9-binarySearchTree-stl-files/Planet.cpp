...
{
	return out << static_cast<const SpaceObject&>(obj)  << obj.rotation_speed << "|" << obj.g << '|' << obj.day_length
	<< "|" << obj.number_of_satelites << "|" << obj.planet_name << "|";
}
...
std::ifstream &operator>>(std::ifstream &in, Planet &obj) {
    ...
    in >> obj.temp;
    in.get();
    in >> obj.rotation_speed;
    in.get();
    in >> obj.g;
    in.get();
    in >> obj.day_length;
    in.get();
    in >> obj.number_of_satelites;
    in.get();
    getline(in, obj.planet_name, '|');
    return in;
}

string Planet::getPath()  {
    return "Planets.txt";
}

string Planet::getPathBinary() {
    return "Planets.bin";
}

void Planet::writeBin(BinaryFile& bin) {
    bin.write(mass);
    bin.write(volume);
    bin.write(temp);
    bin.write(rotation_speed);
    bin.write(g);
    bin.write(day_length);
    bin.write(number_of_satelites);
    size_t size{planet_name.length()};
    bin.write(size);
    bin.writeLine(planet_name);
}

bool Planet::readBin(BinaryFile &bin) {
    if(!bin.in())
        return false;
    bin.read(mass);
    bin.read(volume);
    bin.read(temp);
    bin.read(rotation_speed);
    bin.read(g);
    bin.read(day_length);
    bin.read(number_of_satelites);
    size_t size{};
    bin.read(size);
    planet_name.clear();
    planet_name.reserve(size);
    bin.readLine(planet_name, size);
    return true;
}
